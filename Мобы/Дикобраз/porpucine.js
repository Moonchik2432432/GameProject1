var npcData = {
    sound: {
        ...
        damaged: "customnpcs:porpucine_beg",
        death: "customnpcs:porpucine_death",
        protection: "customnpcs:porpucine_time",
        idle: "customnpcs:porpucine_say"
    },
    animation: {
...
function init(e) {
    ...
    npcTimer.forceStart(10, Math.floor(Math.random() * 180 - 140) + 140, true)
    npc.updateClient();
}
...
function timer(e) {
    ...
    //sound
    if (id === 10 && !deathAnimation) {
        npc.world.playSoundAt(npc.pos, npcData.sound.idle, 0.5, 0.8 + Math.random() * 0.2);
    }
}
...
function damaged(e) {
    ...
    if (!deathAnimation) {
        npc.world.playSoundAt(npc.pos, npcData.sound.damaged, 0.4, 0.8 + Math.random() * 0.2);
    }
    ...
    if (Math.random() <= (npcData.protection.protectionChance / 100) && !isProtectionIdle && !deathAnimation) {
        ...
        npc.world.playSoundAt(npc.pos, npcData.sound.protection, 0.5, 1);
